...
def _accept_move_recalc_alg(curr_F, curr_hess, next_F, next_hess, 
    ...
    This is weighted with Jeffreys prior
    """
    ...
    q_x_to_y = numpy.linalg.det(curr_jtj)*numpy.exp(-_quadratic_cost(step,curr_jtj))
    ...
    q_y_to_x = numpy.exp(-_quadratic_cost(-step,next_jtj))*numpy.linalg.det(next_jtj)
...
def main(argv):
    ...
    bf_file = open('bf_params.dat','rb')
    bf_params=cPickle.load(bf_file)
    bf_file.close()
    
    start_file = open('start_params1.dat','rb')
    ...
    size = 10**8
    fileName = "Ensemble_Jeffreys_"+str(random_seed)
    ens, Fs, ratio=ensemble(jointModule,bf_params,start_params=bf_params,steps=size,step_scale=1,save_hours = 2,recalc_hess_alg=True,save_to = fileName)
